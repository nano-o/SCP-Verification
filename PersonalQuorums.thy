...
We present conditions that are necessary for allowing a subset of the nodes to achieve quorum-based atomic broadcast.
...
proof -
  ...
  proof -
    ...
    moreover
    have \<open>W \<inter> q\<^sub>1 \<inter> q\<^sub>2 \<noteq> {}\<close> if "n\<^sub>1 \<in> S\<^sub>1" and "n\<^sub>2 \<in> S\<^sub>2"
    proof -
    ...
    qed
    moreover
    have \<open>W \<inter> q\<^sub>1 \<inter> q\<^sub>2 \<noteq> {}\<close> if "n\<^sub>2 \<in> S\<^sub>1" and "n\<^sub>1 \<in> S\<^sub>2"
    ...
    proof -
      ...
      qed
      thus "W \<inter> q\<^sub>1 \<inter> q\<^sub>2 \<noteq> {}" by (meson \<open>quorum n\<^sub>1 q\<^sub>1\<close> \<open>self_reliant S\<^sub>2\<close> self_reliant_def \<open>n\<^sub>1 \<in> S\<^sub>2\<close>)  
    qed
    ultimately show "W \<inter> q\<^sub>1 \<inter> q\<^sub>2 \<noteq> {}" using \<open>n\<^sub>1 \<in> S\<^sub>1 \<union> S\<^sub>2\<close> \<open>n\<^sub>2 \<in> S\<^sub>1 \<union> S\<^sub>2\<close> by blast 
  qed
